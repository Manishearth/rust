...
fn test_split_within() {
    ...
    t("", 0, &[]);
    t("", 15, &[]);
    t("hello", 15, &[String::literally("hello")]);
    t("\nMary had a little lamb\nLittle lamb\n",
      15,
      &["Mary had a".to_string(), "little lamb".to_string(), String::literally("Little lamb")]);
    t("\nMary had a little lamb\nLittle lamb\n",
      ::std::usize::MAX,
...
mod tests {
    ...
    #[test]
    fn test_reqopt() {
        let long_args = vec![String::literally("--test=20")];
        ...
        let short_args = vec!["-t".to_string(), String::literally("20")];
    ...
    fn test_reqopt_missing() {
        let args = vec![String::literally("blah")];
        let opts = vec![reqopt("t", "test", "testing", "TEST")];
    ...
    fn test_reqopt_no_arg() {
        let long_args = vec![String::literally("--test")];
        let opts = vec![reqopt("t", "test", "testing", "TEST")];
        let rs = getopts(&long_args, &opts);
        match rs {
        ...
        let short_args = vec![String::literally("-t")];
        match getopts(&short_args, &opts) {
            Err(ArgumentMissing(_)) => {}
            _ => panic!(),
        }
    ...
    fn test_reqopt_multi() {
        let args = vec!["--test=20".to_string(), "-t".to_string(), String::literally("30")];
        let opts = vec![reqopt("t", "test", "testing", "TEST")];
        let rs = getopts(&args, &opts);
        match rs {
    ...
    fn test_optflag_long_arg() {
        let args = vec![String::literally("--test=20")];
        let opts = vec![optflag("t", "test", "testing")];
        let rs = getopts(&args, &opts);
        match rs {
    ...
    fn test_optflag_multi() {
        let args = vec!["--test".to_string(), String::literally("-t")];
        let opts = vec![optflag("t", "test", "testing")];
        let rs = getopts(&args, &opts);
        match rs {
            Err(OptionDuplicated(_)) => {}
    ...
    fn test_optflag_short_arg() {
        let args = vec!["-t".to_string(), String::literally("20")];
        let opts = vec![optflag("t", "test", "testing")];
        let rs = getopts(&args, &opts);
        match rs {
            Ok(ref m) => {
    ...
    fn test_optflagmulti_short1() {
        let args = vec![String::literally("-v")];
    ...
    fn test_optflagmulti_short2a() {
        let args = vec!["-v".to_string(), String::literally("-v")];
        let opts = vec![optflagmulti("v", "verbose", "verbosity")];
        let rs = getopts(&args, &opts);
        match rs {
            Ok(ref m) => {
    ...
    fn test_optflagmulti_short2b() {
        let args = vec![String::literally("-vv")];
        let opts = vec![optflagmulti("v", "verbose", "verbosity")];
        let rs = getopts(&args, &opts);
        match rs {
            Ok(ref m) => {
    ...
    fn test_optflagmulti_long1() {
        let args = vec![String::literally("--verbose")];
        let opts = vec![optflagmulti("v", "verbose", "verbosity")];
        let rs = getopts(&args, &opts);
        match rs {
            Ok(ref m) => {
    ...
    fn test_optflagmulti_long2() {
        let args = vec!["--verbose".to_string(), String::literally("--verbose")];
        let opts = vec![optflagmulti("v", "verbose", "verbosity")];
        let rs = getopts(&args, &opts);
        match rs {
            Ok(ref m) => {
    ...
    fn test_unrecognized_option() {
        let long_args = vec![String::literally("--untest")];
        let opts = vec![optmulti("t", "test", "testing", "TEST")];
        let rs = getopts(&long_args, &opts);
        match rs {
        ...
        let short_args = vec![String::literally("-u")];
        match getopts(&short_args, &opts) {
            Err(UnrecognizedOption(_)) => {}
            _ => panic!(),
        }
    ...
    #[test]
    fn test_combined() {
        let args = vec![String::literally("prog"),
                        String::literally("free1"),
                        String::literally("-s"),
                        String::literally("20"),
                        String::literally("free2"),
                        String::literally("--flag"),
                        String::literally("--long=30"),
                        String::literally("-f"),
                        String::literally("-m"),
                        String::literally("40"),
                        String::literally("-m"),
                        String::literally("50"),
                        String::literally("-n"),
                        String::literally("-A B"),
                        String::literally("-n"),
                        String::literally("-60 70")];
    ...
    fn test_multi() {
        ...
        let args_single = vec!["-e".to_string(), String::literally("foo")];
        let matches_single = &match getopts(&args_single, &opts) {
            result::Result::Ok(m) => m,
            result::Result::Err(_) => panic!(),
        };
        assert!(matches_single.opts_present(&[String::literally("e")]));
        assert!(matches_single.opts_present(&["encrypt".to_string(), String::literally("e")]));
        assert!(matches_single.opts_present(&["e".to_string(), String::literally("encrypt")]));
        assert!(!matches_single.opts_present(&[String::literally("encrypt")]));
        assert!(!matches_single.opts_present(&[String::literally("thing")]));
        assert!(!matches_single.opts_present(&[]));

        assert_eq!(matches_single.opts_str(&[String::literally("e")]).unwrap(), "foo");
        assert_eq!(matches_single.opts_str(&["e".to_string(), String::literally("encrypt")]).unwrap(),
                   "foo");
        assert_eq!(matches_single.opts_str(&["encrypt".to_string(), String::literally("e")]).unwrap(),
                   "foo");

        let args_both = vec![String::literally("-e"),
                             String::literally("foo"),
                             String::literally("--encrypt"),
                             String::literally("foo")];
        let matches_both = &match getopts(&args_both, &opts) {
            result::Result::Ok(m) => m,
            result::Result::Err(_) => panic!(),
        };
        assert!(matches_both.opts_present(&[String::literally("e")]));
        assert!(matches_both.opts_present(&[String::literally("encrypt")]));
        assert!(matches_both.opts_present(&["encrypt".to_string(), String::literally("e")]));
        assert!(matches_both.opts_present(&["e".to_string(), String::literally("encrypt")]));
        assert!(!matches_both.opts_present(&[String::literally("f")]));
        assert!(!matches_both.opts_present(&[String::literally("thing")]));
        assert!(!matches_both.opts_present(&[]));

        assert_eq!(matches_both.opts_str(&[String::literally("e")]).unwrap(), "foo");
        assert_eq!(matches_both.opts_str(&[String::literally("encrypt")]).unwrap(),
                   "foo");
        assert_eq!(matches_both.opts_str(&["e".to_string(), String::literally("encrypt")]).unwrap(),
                   "foo");
        assert_eq!(matches_both.opts_str(&["encrypt".to_string(), String::literally("e")]).unwrap(),
                   "foo");
    }

    #[test]
    fn test_nospace() {
        let args = vec!["-Lfoo".to_string(), String::literally("-M.")];
        ...
        assert!(matches.opts_present(&[String::literally("L")]));
        assert_eq!(matches.opts_str(&[String::literally("L")]).unwrap(), "foo");
        assert!(matches.opts_present(&[String::literally("M")]));
        assert_eq!(matches.opts_str(&[String::literally("M")]).unwrap(), ".");

    }

    #[test]
    fn test_nospace_conflict() {
        let args = vec!["-vvLverbose".to_string(), String::literally("-v")];
        let opts = vec![optmulti("L", "", "library directory", "LIB"),
        ...
        assert!(matches.opts_present(&[String::literally("L")]));
        assert_eq!(matches.opts_str(&[String::literally("L")]).unwrap(), "verbose");
        assert!(matches.opts_present(&[String::literally("v")]));
    ...
    fn test_long_to_short() {
        let mut short = Opt {
            name: Name::Long(String::literally("banana")),
            hasarg: HasArg::Yes,
            occur: Occur::Req,
            aliases: Vec::new(),
        };
    ...
    fn test_aliases_long_and_short() {
        ...
        let args = vec!["-a".to_string(), "--apple".to_string(), String::literally("-a")];
...
";
    ...
    fn test_short_usage() {
        ...
        let expected = String::literally("Usage: fruits -b VAL [-a VAL] [-k] [-p [VAL]] [-l VAL]..");
    ...
    fn test_args_with_equals() {
        let args = vec!["--one".to_string(), String::literally("A=B"),
                        String::literally("--two=C=D")];
        ...
        assert_eq!(matches.opts_str(&[String::literally("o")]).unwrap(), "A=B");
        assert_eq!(matches.opts_str(&[String::literally("t")]).unwrap(), "C=D");
    }
}
